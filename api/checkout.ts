...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (body.action === "portal") {
      ...
      if (!customerId) {
        return res.status(404).json({ error: "No Stripe customer on file. Open a support ticket in our Discord (https://discord.gg/CNpWqkFA65) or email gltch.app@proton.me to cancel." });
      }
